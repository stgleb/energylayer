 ...
 * Modified for use in MoonCake while waiting for the final version of jQuery-UI 1.9 Spinner
 *
 * Changes:
 ...
 * 4. Converted all class names for Bootstrap use
 *
 */
...
$.widget('ui.spinner', {
	...
	_createButtons: function(input) {
		var self = this,
			...
			wrapper = self.wrapper = input
				.wrap('<span class="spinner-container"></span>').parent(),
			btnContainer = self.btnContainer = $(
				'<span class="spinner-btn-group btn-group btn-group-vertical">' + 
					'<span class="btn-spinner-up btn"><span class="'+options.upIconClass+'"></span></span>' + 
					'<span class="btn-spinner-down btn"><span class="'+options.downIconClass+'"></span></span>' + 
				'</span>'),
		...
		buttons = self.buttons = btnContainer.find('.btn');
		upButton = buttons[0];
	...
	destroy: function(target) {
		this.wrapper.remove();
		this.element.unbind(eventNamespace).removeClass('spinner-input');
